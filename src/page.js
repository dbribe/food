(function () {
  ...
  /*#__PURE__*/
  function (_BasePlatform) {
    _inherits(FoodPanda, _BasePlatform);

    function FoodPanda() {
      _classCallCheck(this, FoodPanda);

      return _possibleConstructorReturn(this, _getPrototypeOf(FoodPanda).apply(this, arguments));
    }

    _createClass(FoodPanda, [{
      key: "getDishes",
      value: function getDishes() {
        var elements = document.querySelectorAll(".dish-card");
        return _toConsumableArray(elements).map(function (element) {
          return {
            element: element,
  ...
  window.addEventListener("foodMessage", function (event) {
    var getMerchantOrder = function getMerchantOrder() {
      return window.location.host === "www.caserola.ro" ? Caserola : window.location.host === "sushi-terra.ro" ? SushiTerra : window.location.host === "www.foodpanda.ro" ? FoodPanda : TakeAway;
    };
